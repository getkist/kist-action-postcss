# AGENTS.md — kist-action-postcss

`@getkist/action-postcss` is a **kist action plugin**: an npm package whose
default export is an `ActionPlugin` descriptor that kist core loads to register
actions usable from a `kist.yml` pipeline. PostCSS processing actions for kist.
...
| --- | --- |
| `PostCssAction` | Processes CSS through PostCSS with autoprefixer and cssnano. |

Runtime dependencies: `autoprefixer`, `cssnano`, `postcss`.
...
- Overlaps with `@getkist/action-sass`, which embeds the same PostCSS pipeline
  behind its SCSS compile. Fix a PostCSS behaviour bug in both, or neither.
...
```bash
npm run build
# then, from a project with a kist.yml that uses PostCssAction:
